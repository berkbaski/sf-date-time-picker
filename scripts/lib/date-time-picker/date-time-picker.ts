...
import { IDatePicker, IHourPicker } from 'lib/date-time-picker/types';

export function createDatePicker(options: IDatePicker): FlexLayout | Dialog {
    const momentObject = moment();
    let currentMoment = moment();
    const processResult = () => {
        const dateFormat = getDateString({
        ...
        currentMoment = moment(dateFormat, DATE_FORMAT);
        if (options.textBox) {
    ...
    const startYear = options.startYear || PICKER_YEAR_RANGE;
    ...
    const endDay = options.endDay || moment({ month: currentMonthIndex }).endOf('month').day();
    const endMonth = options.startMonth || momentObject.endOf('year').month();

    const days = getDays({ month: currentMonthIndex, year: currentYear });
    const daysInNumber = getDays({ month: currentMonthIndex, year: currentYear, format: "D" }).map((day) => Number(day));
    ...
    const dayPicker = createPicker(days);
    const monthPicker = createPicker(months);
    const yearPicker = createPicker(years);
    ...
    monthPicker.onSelected = (index: number) => {
        ...
        const newDays = getDays({ month: moment().month(selectedMonth).month(), year: Number(yearPicker.items[yearPicker.currentIndex]) }) as [];
    ...
    const flDayPicker = createPickerLayout({ picker: dayPicker, label: global.lang.day });
    const flMonthPicker = createPickerLayout({ picker: monthPicker, label: global.lang.month });
    const flYearPicker = createPickerLayout({ picker: yearPicker, label: global.lang.year });
    const rootLayout = createPickerRootLayout([flDayPicker, flMonthPicker, flYearPicker], { onPickerDone: pickerDone, skipInputView: !options.textBox });
...
export function createHourPicker(options: IHourPicker): FlexLayout | Dialog {
    ...
    const processResult = () => {
        ...
        currentMoment = moment(hourFormat, HOUR_FORMAT);
        if (options.textBox) {
            options.textBox.text = currentMoment.format(HOUR_FORMAT);
        }
    };
    ...
    const hours = getRange(startHour, endHour, 1).map((hour) => getIntegerDoubleDigit(hour));
    const minutes = getRange(startMinute, endMinute, minutePeriod).map((minute) => getIntegerDoubleDigit(minute));
    const hourPicker = createPicker(hours);
    const minutePicker = createPicker(minutes);
    hourPicker.currentIndex = Math.max(hours.indexOf(getIntegerDoubleDigit(currentHour)), 0);
    minutePicker.currentIndex = Math.max(minutes.indexOf(getIntegerDoubleDigit(currentMinute)), 0);
    ...
    const flHourPicker = createPickerLayout({ picker: hourPicker, label: global.lang.hour });
    const flMinutePicker = createPickerLayout({ picker: minutePicker, label: global.lang.minute });
    const rootLayout = createPickerRootLayout([flHourPicker, flMinutePicker], {
 ...
 */
function createPickerRootLayout(pickerLayouts: FlexLayout[], options: {
    ...
    const { backgroundColor, alignItems, justifyContent } = getCombinedStyle('.dialog');
    ...
    const flDialogButtons = createDialogButtons(onPickerDone, onPickerCancel);
...
function createDialogButtons(onDonePress: (...args: any[]) => void, onCancelPress: () => void): FlexLayout {
    ...
    const btnOK = new Button({
        text: System.OS === System.OSType.IOS ? global.lang.done : global.lang.ok,
 ...
 */
function createPickerLayout(options: { picker: Picker, label: string }): FlexLayout {
...
function createPicker(items: Array<string>): Picker {
...
function getRange(start: number, stop: number, step = 1): string[] {
 ...
 */
export function getDays(options: { month?: number, year?: number, format?: string }): string[] {
    const monthToProcess = options.month || moment().month();
    const yearToProcess = options.year || moment().year();
    const formatToProcess = options.format || DAY_WITH_NAME_FORMAT;
 ...
 */
export function getYears(options?: { startYear?: number, endYear?: number }): string[] {
    const startYearRange = (options && options.startYear) ? options.startYear : MOMENT_START_YEAR;
    const endYearRange = (options && options.endYear) ? options.endYear : moment().year();

    return getRange(endYearRange, startYearRange, -1);
}

function getDateString({ year = 1970, month = 1, day = 1 }) {
    return moment({ year, month, day }).format(DATE_FORMAT);
}

function getHourString({ hour = 0, minute = 0 }) {
    return moment({ hour, minute }).format(HOUR_FORMAT);
}

function getIntegerDoubleDigit(numberString) {
